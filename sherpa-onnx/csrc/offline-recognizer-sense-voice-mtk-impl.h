// sherpa-onnx/csrc/offline-recognizer-sense-voice-mtk-impl.h
//
// Copyright (c)  2024  Xiaomi Corporation
// Copyright (c)  2024  MediaTek Inc.

#ifndef SHERPA_ONNX_CSRC_OFFLINE_RECOGNIZER_SENSE_VOICE_MTK_IMPL_H_
#define SHERPA_ONNX_CSRC_OFFLINE_RECOGNIZER_SENSE_VOICE_MTK_IMPL_H_
...
class OfflineRecognizerSenseVoiceMtkImpl : public OfflineRecognizerImpl {
 public:
  explicit OfflineRecognizerSenseVoiceMtkImpl(
      const OfflineRecognizerConfig &config)
      : OfflineRecognizerImpl(config),
        config_(config),
        symbol_table_(config_.model_config.tokens),
        model_(
            std::make_unique<OfflineSenseVoiceModelMtk>(config.model_config)) {
  ...
  OfflineRecognizerSenseVoiceMtkImpl(Manager *mgr,
                                      const OfflineRecognizerConfig &config)
      : OfflineRecognizerImpl(mgr, config),
        config_(config),
        symbol_table_(mgr, config_.model_config.tokens),
        model_(std::make_unique<OfflineSenseVoiceModelMtk>(
 ...
 private:
  ...
  std::unique_ptr<OfflineSenseVoiceModelMtk> model_;
  std::unique_ptr<OfflineCtcGreedySearchDecoderMtk> decoder_;
};

}  // namespace sherpa_onnx

#endif  // SHERPA_ONNX_CSRC_OFFLINE_RECOGNIZER_SENSE_VOICE_MTK_IMPL_H_
