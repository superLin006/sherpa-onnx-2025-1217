#!/usr/bin/env bash
# Build sherpa-onnx (this 1.12.17 tree) with the Sophon BM1684X backend, inside
# the sophon-cross-build container (Ubuntu 20.04 + aarch64-linux-gnu GCC 9.4 +
# glibc 2.31), which matches the BM1684X board (Ubuntu 20.04).
#
# Run from the host:
#   docker run --rm -v "$PWD":/repo -v <soc-sdk>:/sdk -w /repo \
#       sophon-cross-build:latest bash build-sophon-docker.sh
set -ex
